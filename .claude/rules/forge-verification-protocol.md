...
# Forge Verification Protocol

## Core Principle

Never verify your own output in the same context.

- **Internal consistency**: Checked by `forge verify` (deterministic) + Agent(verifier) (separate context)
...
- **Code references**: Checked by `forge verify` when code exists (deterministic, optional)
