...
3. Log to changelog:
   ```bash
   forge recipe log {id} --action adjudicate --result "extend: [reasons]"
   ```
...
4. Kick off a follow-up debate:
   ...
   - Use Skill("forge-debate") with this combined context as arguments
...
Log:
```bash
forge recipe log {id} --action adjudicate --output reservations.md --result "accepted with reservations after max extensions"
```

## Output

Always output one of:
- `<forge>ADJUDICATE: ACCEPT</forge>` — conclusion is solid, proceed
- `<forge>ADJUDICATE: EXTEND N/3</forge>` — another round needed, preparation brief attached
- `<forge>ADJUDICATE: ACCEPT WITH RESERVATIONS</forge>` — max extensions reached, proceed with caveats
